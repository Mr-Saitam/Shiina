...
module.exports = {
	map: function (clave, callback, split_object, global_object, self) {
	  ...
	  check_path_object(clave, split_object, 'map', callback)

	  var [...args] = clave.split(split_object)
		var object_data = global_object[self.object_id]
		
	  for(var key of args) {
			if(!object_data.hasOwnProperty(key)) return Promise.reject(new errorDB(`No se encontro la propiedad ${key}, base de datos: ${self.database_name}`, 'PROPIEDAD NO EXISTENTE', 'map', clave, callback))
